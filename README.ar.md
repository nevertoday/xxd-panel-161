...
# XXD Panel 161｜حقل لوني مرسوم يتجاوز الإطار
...
وُلّدت هذه النماذج الثمانية بواسطة Panel 161، كل منها بمحاولة واحدة ومن صورة مصدر مختلفة مع نص إنجليزي قصير. ضُبطت الأبعاد إلى 1536×864 بتقسيم متساوٍ يميناً ويساراً و1152×1536 بتقسيم متساوٍ أعلى وأسفل، وتم التحقق من تنظيف البيانات الوصفية. هذه نتائج فعلية وليست تأكيداً لاجتياز جميع متطلبات الأسلوب.

انحرافات ملحوظة: بعض المساحات اللونية والعناصر كبيرة مع احتفاظ زائد بتفاصيل المشهد، وبعض علاقات تجاوز الإطار ضعيفة.
...
حقل مستطيل غير منتظم داخل هوامش ورقية واسعة، تتجاوز أجزاء قليلة من الموضوع حدوده المرسومة.
...
- `design-only`: تعرض اللوحة الكاملة ترجمة Panel 161 التصميمية فقط، وتبقى الصورة مرجعاً غير ظاهر.
...
بعد التثبيت أعد تشغيل جلسة Agent ثم استدعِ `$xxd-panel-161`. ويمكن إضافة `--global --agent codex --yes` للتثبيت على مستوى المستخدم.

```text
/xxd-panel-161 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ar-SA
...
راجع [SKILL.md](SKILL.md) لعقد التشغيل الكامل، ومهايئ التشغيل [بالإنجليزية](references/xxd-panel-161-prompt.en.md) أو [بالصينية](references/xxd-panel-161-prompt.zh-CN.md).
