...
異なる元写真から Panel 161 で各1回生成した8点です。短い英文を使用し、横1536×864・左右均等、縦1152×1536・上下均等に整え、メタデータ清掃を検証しました。実際のモデル出力を残しており、全要件への適合を保証するものではありません。

確認された偏差：色面や主題が大きく、場面情報が残りすぎる例があり、枠を越える構成が弱い例もあります。
...
インストール後に Agent セッションを再起動し、`$xxd-panel-161` を呼び出します。ユーザー単位の Codex には `--global --agent codex --yes` を追加できます。

```text
/xxd-panel-161 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ja-JP
...
完全な実行契約は [SKILL.md](SKILL.md)、実行アダプターは[英語](references/xxd-panel-161-prompt.en.md)／[中国語](references/xxd-panel-161-prompt.zh-CN.md)を参照してください。
