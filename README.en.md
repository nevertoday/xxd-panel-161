...
# XXD Panel 161｜Frame-breaking Painted Colour Field
...
Native PNGs are retained without resizing, cropping or split correction. Landscapes are 1672×941 (approximately 16:9); portraits are 1086×1448 (3:4). Landscape boundaries are near the midpoint; the source region in these portraits occupies approximately 44.6–50.0%. These are not all certified exact 50:50 splits.

Every full-size image and thumbnail was reviewed, and metadata cleanup was verified. These are actual outputs, **not a fully accepted style showcase**. Some colour fields and subjects are oversized, with excessive scene detail and weak frame-breaking relationships.
...
Place an irregular hand-painted rectangular field inside wide paper margins. Organise most of the simplified hand-drawn subject within it and let a few meaningful contours cross its brushed, broken edges. Keep a small focus, vast whitespace and 2–4 fresh source-derived colours. Avoid full-bleed fields, hard template borders and literal scene copying.
...
- `design-only`: the full canvas contains only Panel 161's designed translation; the photograph remains a non-visible reference.
...
```bash
git clone https://github.com/nevertoday/xxd-panel-161.git
npx skills add https://github.com/nevertoday/xxd-panel-161 --skill xxd-panel-161
```

Restart the agent session after installation, then invoke `$xxd-panel-161`. Add `--global --agent codex --yes` when a user-level Codex installation is wanted.

Common examples:

```text
/xxd-panel-161 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale en-US
/xxd-panel-161 photo.jpg --mode left-right --size 16:9 --text prompt --locale en-US
/xxd-panel-161 photo.jpg --mode design-only --size 9:16 --text none
/xxd-panel-161 ./photos --mode design-only --size auto,3:4 --text prompt --locale ja-JP
```

See [SKILL.md](SKILL.md) for the full runtime contract and the [English](references/xxd-panel-161-prompt.en.md) or [Chinese](references/xxd-panel-161-prompt.zh-CN.md) runtime adapter.
