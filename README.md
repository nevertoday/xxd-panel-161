...
以下 8 张样张由 Panel 161 基于不同源照片独立单轮生成，使用简短英文文案。已归整为横版 1536×864、左右各 50%，及竖版 1152×1536、上下各 50%，并完成元数据清理。保留单次模型结果供观察，不代表全部通过风格验收。

已观察到的偏差：部分色域和主体偏大，场景信息保留偏多；部分破框关系较弱。
...
不规则手工矩形色场如第二画布，四周大白边，少量主体越界，2–4色概括手绘。
...
- `design-only`：整张画布只呈现 Panel 161 的设计转译，照片只作为不可见参考。
...
安装后重新启动 Agent 会话，然后调用 `$xxd-panel-161`。也可以按需追加 `--global --agent codex --yes` 做用户级安装。

常用调用示例：

```text
/xxd-panel-161 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale zh-CN
...
完整运行契约见 [SKILL.md](SKILL.md)；运行适配器见 [英文](references/xxd-panel-161-prompt.en.md) 与 [中文](references/xxd-panel-161-prompt.zh-CN.md)。
...
Input image for this panel: `assets/inputs/input-03`

![Re-selected input](assets/inputs/input-03.jpg)
